...
var Features = {
    evaluateFeatures: function (sketch) {
        ...
        console.log(numTriangles + " " + feature_count);

        features_arr[feature_count] = [];
        features_arr[feature_count++] = [numTriangles, lines.length, graphEdges.length, isPolyline, numberOfCorners, bbDiagLen, absRotation, /*smoothness,*/ strokeLen,/* curviness,distanceBetweenStartAndEnd, openness, dcr, */ ndde, sketch.interpretation];
        
        var cnt = 0;

        if(numTriangles > 0) cnt++;


        //console.log(sketch.strokes.strokeIds);

        var output = { 'containsTruss': false, 'trussStrokeIds': [] };
        output.containsTruss = cnt > 1 ? true : false ;
        output.strokeIds = " ";
        return output;
    },
    initialize: function () {
    ...
    detectTriangles: function (adjMat) {
        var c = 0;
        //i+1 is end point for i
        for (var i = 0; i < adjMat.length; i = i + 2) 
        {
            for (var j = 0; j < adjMat[i].length - 2; j++) 
            {
                for (var k = 0; k < adjMat[i+1].length - 2; k++) 
                {
                    if (this.distance(adjMat[i+1][k], adjMat[i][j]) <= closenessThreshold) 
                    {
                        //second last point is the same point
                        //last element in adj is the end point of lines
                        side1 = this.distance(adjMat[i][adjMat[i].length - 2], adjMat[i][j]);
                        side2 = this.distance(adjMat[i+1][adjMat[i+1].length - 2], adjMat[i+1][k]);
                        side3 = this.distance(adjMat[i][adjMat[i].length - 2], adjMat[i][adjMat[i].length - 1]);
    ...
    displayCornerFindingIStraw: function (sketch) {
        // get the resampled sketch and its corner indices
        var resampledSketch = SketchRecTools.resampleByDistance(sketch);
        closenessThreshold = 0.238*this.bbDiagLen(sketch); //sampleSpace; //defined in sketch rec tools
        ...
        for (var i = 0; i < resampledSketch.strokes.length; i++) {
            ...
            var strokeCornerIndices = sketchCornerIndices[i];
            // if(this.checkForStraightLine(resampledPoints, strokeCornerIndices))
            //     cnt++;
            isPolyline = isPolyline && checkForStraightLine;
            for (var j = 0; j < strokeCornerIndices.length; j++) {
